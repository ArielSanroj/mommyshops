...
from google_vision_integration import extract_ingredients_with_google_vision
        ...
        try:
            if ollama_integration.is_available():
                ...
                try:
                    ...
                    prompt = f"""
                    Analiza esta imagen de producto cosmético y extrae la lista de ingredientes.

                    Información disponible:
                    - Marca: {product_info.brand or 'Desconocida'}
                    - Producto: {product_info.product_name or 'Desconocido'}
                    - Tipo: {product_info.product_type or 'Desconocido'}

                    Devuelve solo una lista separada por comas de ingredientes INCI.
                    Si no logras leerlos, responde exactamente "No ingredients visible".
                    """

                    ollama_result = await ollama_integration.analyze_ingredients([prompt])
                    if ollama_result.success and ollama_result.content:
                        response_text = ollama_result.content.strip()
                        if "no ingredients visible" in response_text.lower():
                            return []
                        ingredients = [ing.strip() for ing in response_text.split(",") if ing.strip()]
                        if ingredients:
                            logger.info("Ollama LLM extracted %d ingredients", len(ingredients))
                            return canonicalize_ingredients(ingredients)
                except Exception as exc:
                    logger.warning("Ollama LLM analysis failed: %s", exc)

            logger.info("Falling back to Google Vision OCR for ingredient extraction.")
            ingredients = await extract_ingredients_with_google_vision(image_data)
            return canonicalize_ingredients(ingredients)
        except Exception as exc:
            logger.error("LLM/OCR analysis failed: %s", exc)
            return []
