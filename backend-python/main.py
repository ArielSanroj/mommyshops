...
async def lifespan(app: FastAPI):
    ...
    if os.getenv("SKIP_STARTUP_TASKS", "0") not in {"1", "true", "yes"}:
        try:
...
# Include Java integration router
if os.getenv("ENABLE_JAVA_ROUTER", "false").lower() in {"1", "true", "yes"}:
    from java_integration_endpoints import java_router
    app.include_router(java_router)

RECOMMENDER = IngredientRecommender()
RECOMMENDER.configure(SessionLocal)

# Global security configuration (rate limiting + log sanitization)
    ...
    *,
    ...
    if user_profile and base_suggestions:
        try:
            ...
            logger.error("Ollama substitution refinement failed: %s", exc)
...
async def analyze_routine_endpoint(
    ...
    ollama_enrichment_cache: Dict[str, Dict[str, Any]] = {}
    ...
    try:
        ...
        for product in product_inputs:
            ...
            ollama_alternatives: List[str] = []

            excluded_ingredients = analysis_data.get("excluded", []) or []
            if excluded_ingredients:
                for flagged in excluded_ingredients[:2]:
                    key = flagged.lower()
                    if key not in ollama_enrichment_cache:
                        try:
                            ollama_enrichment_cache[key] = await enrich_ingredient_with_ollama(
                                flagged,
                                profile.get("conditions", []) if profile else []
                            )
                        except Exception as exc:
                            logger.error("Ollama enrichment failed for %s: %s", flagged, exc)
                            ollama_enrichment_cache[key] = {}

                    enrichment = ollama_enrichment_cache.get(key) or {}
                    ...
                    for alternative in alternatives:
                        ...
                        if label:
                            ollama_alternatives.append(label)
            ...
            if not meets_needs:
                try:
                    ...
                    for item in substitutes_raw:
                        ...
                        if reason_text:
                            ollama_alternatives.append(f"{suggestion.name}: {reason_text}")
                        else:
                            ollama_alternatives.append(suggestion.name)
...
async def analyze_ingredients_and_recommend(
    ...
    if payload.user_id and payload.user_id.isdigit():
        requested_id = int(payload.user_id)
        if requested_id != current_user.id:
            raise HTTPException(status_code=403, detail="Solo puedes analizar tus propios ingredientes")
        try:
            target_user = db.query(User).filter(User.id == requested_id).first()
        except Exception as exc:
...
@app.get("/debug/env")
async def debug_env():
    """Debug environment variables (sanitized for security)."""
    # Sanitize sensitive environment variables
    sensitive_keys = ['PASSWORD', 'SECRET', 'KEY', 'TOKEN', 'API_KEY', 'MYSQL', 'POSTGRES', 'REDIS']
